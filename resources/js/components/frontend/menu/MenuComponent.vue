<template>
    ...
    <section class="mb-24 sm:mb-16 mt-4 sm:mt-8">
        <div class="container">
            ...
            <div v-if="categories.length > 0" class="flex flex-wrap gap-3 w-full mb-6 sm:mb-12 veg-navs">
                <button :disabled="itemProps.type !== null && itemProps.type === itemTypeEnum.VEG"
                    ...
                    class="flex items-center gap-3 w-fit pl-3 pr-4 py-1 sm:py-1.5 rounded-3xl transition hover:shadow-filter hover:bg-white bg-[#EFF0F6]">
                    <img :src="setting.image_vag" alt="category" class="h-6">
                    <span class="capitalize text-xs sm:text-sm font-medium text-heading">{{ $t('label.frontend_non_veg')
                        }}</span>
                    <i
                        class="lab-close-circle-line text-lg sm:text-xl text-red-500 transition opacity-0 ltr:-ml-8 rtl:-mr-8 clear-item-type-filter font-fill-danger"></i>
                </button>
                <button :disabled="itemProps.type !== null && itemProps.type === itemTypeEnum.NON_VEG"
                    ...
                    <span class="capitalize text-xs sm:text-sm font-medium text-heading">{{ $t('label.veg') }}</span>
                    <i
                        class="lab-close-circle-line text-lg sm:text-xl text-red-500 transition opacity-0 ltr:-ml-8 rtl:-mr-8 font-fill-danger"></i>
                </button>
            </div>

            <div v-if="Object.keys(category).length > 0"
                class="flex gap-2 sm:gap-4 items-start justify-between mb-4 sm:mb-6">
