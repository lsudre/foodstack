    ...
    actions: {
        ...
        save: function (context, payload) {
            return new Promise((resolve, reject) => {
                ...
                let url = "/admin/waiter";
                if (this.state["waiter"].temp.isEditing) {
                    method = axios.post;
                    url = `/admin/waiter/${this.state["waiter"].temp.temp_id}`;
                }
        ...
        destroy: function (context, payload) {
            return new Promise((resolve, reject) => {
                axios
                    .delete(`admin/waiter/${payload.id}`)
                    .then((res) => {
                        context
                            .dispatch("lists", payload.search)
                            .then()
        ...
        changePassword: function (context, payload) {
            return new Promise((resolve, reject) => {
                axios
                    .post(
                        `/admin/waiter/change-password/${payload.id}`,
                        payload.form
                    )
                    .then((res) => {
                        resolve(res);
        ...
        changeImage: function (context, payload) {
            return new Promise((resolve, reject) => {
                axios
                    .post(
                        `/admin/waiter/change-image/${payload.id}`,
                        payload.form
                    )
                    .then((res) => {
                        context.commit("show", res.data.data);
        ...
        myOrders: function (context, payload) {
            return new Promise((resolve, reject) => {
                let url = `admin/waiter/my-order/${payload.id}`;
                if (payload.search) {
